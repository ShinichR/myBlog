...
func (this *ArticleController) List() {
	...
	beego.Debug("pagesize=", pagesize, "offset=", offset)
	...
	this.Data["list"] = list
	this.Data["pagebar"] = 1 //util.NewPager(page, int(count), pagesize, fmt.Sprintf("/admin/article/list?status=%d&searchtype=%s&keyword=%s", status, searchtype, keyword), true).ToString()
	this.display()
}
...
func (this *ArticleController) Save() {
	...
	post.UpdateTime = this.getTime()
	//post.Update( "title", "color", "is_top", "content", "url_name", "url_type", "updated", "post_time")

	post.Update("tags", "status", "title", "color", "is_top", "content", "url_name", "url_type", "post_time")
